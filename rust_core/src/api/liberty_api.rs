...
use crate::ranking::ranking_service::{RankingService, RankEntry};
use std::sync::Arc;
...
pub struct LibertyCore {
    ...
    pub ranking: Option<RankingService>,
...
pub async fn init(peer_name: String, localai_url: String, storage_path: String) -> String {
    ...
    let ranking = RankingService::new(storage.clone());
    core.swarm = Some(swarm);
    core.ai = Some(ai);
    core.storage = Some(storage);
    core.ranking = Some(ranking);
...
#[flutter_rust_bridge::frb]
pub async fn get_leaderboard(period: String, limit: u32) -> Vec<RankEntry> {
    let core = INSTANCE.read().await;
    match &core.ranking {
        Some(r) => r.get_leaderboard(&period, limit).await.unwrap_or_default(),
        None => vec![],
    }
}

/// Record a point-earning activity
#[flutter_rust_bridge::frb]
pub async fn record_activity(user_id: String, display_name: String, points: u64) -> String {
    let core = INSTANCE.read().await;
    match &core.ranking {
        Some(r) => match r.record_activity(&user_id, &display_name, points).await {
            Ok(_) => format!("Recorded {} points for {}", points, user_id),
            Err(e) => format!("Error: {}", e),
        },
        None => "Ranking not initialized".to_string(),
    }
}

/// Get my rank and total points
#[flutter_rust_bridge::frb]
pub async fn get_my_rank(user_id: String) -> (u32, u64) {
    let core = INSTANCE.read().await;
    match &core.ranking {
        Some(r) => r.get_my_rank(&user_id).await.unwrap_or((0, 0)),
        None => (0, 0),
    }
}
...
#[flutter_rust_bridge::frb]
pub async fn get_user_points(user_id: String) -> crate::ranking::ranking_service::UserPoints {
    let core = INSTANCE.read().await;
    match &core.ranking {
        Some(r) => r.get_user_points(&user_id).await.unwrap_or(
            crate::ranking::ranking_service::UserPoints {
        ...
        None => crate::ranking::ranking_service::UserPoints {
            user_id: user_id.clone(),
            display_name: String::new(),
            total_points: 0,
            monthly_points: 0,
